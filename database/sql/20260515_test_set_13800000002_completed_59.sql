-- Repair test data only.
-- Goal: tel 13800000002 has exactly 59 completed test orders today.
-- This script only deletes/recreates orders whose order_no starts with T13800000002_.
-- It does NOT move or modify this user's other orders.
...
SET @today_start := UNIX_TIMESTAMP(CURDATE());
...
WHERE `user_id` = @uid
  AND `order_no` LIKE 'T13800000002\_%';
...
SELECT
  ...
  CONCAT('T13800000002_', LPAD(n.n, 2, '0')),
  @goods_id,
  1,
  @goods_name,
  @goods_name,
  ...
  'test_completed_59',
  '1',
  'test set today completed count to 59',
  @now - 3600 + n.n,
  @now - 1800 + n.n,
  @now - 3600 + n.n,
  @now
...
SELECT
  @uid AS `user_id`,
  COUNT(*) AS `test_completed_count`
FROM `fa_miniapp_order`
WHERE `user_id` = @uid
  AND `status` = 2
  AND `complete_time` >= @today_start
  AND `order_no` LIKE 'T13800000002\_%';

SELECT
  @uid AS `user_id`,
  COUNT(*) AS `all_completed_count_today`
FROM `fa_miniapp_order`
WHERE `user_id` = @uid
  AND `status` = 2
  AND `complete_time` >= @today_start;
